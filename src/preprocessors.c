...
struct csm_shader_t* find_shader(const char* name)
{
  ...
  return (struct csm_shader_t*) bsearch(&shader, shader_map, nshaders, sizeof(struct csm_shader_t),
                                        compare_shader_t);
}
...
int csm_load_shader(const char* name, struct csm_shader_t** out)
{
  ...
  struct csm_shader_t* shaderptr = (struct csm_shader_t*) bsearch(
      &shader, shader_map, nshaders, sizeof(struct csm_shader_t), compare_shader_t);
...
void csm_register_all_shaders(VkDevice vkdevice, struct csm_shader_entry* entries, int nentries)
{
  ...
  if (shader_map)
  {
    luna_memcpy(new_shader_map, nshaders * sizeof(struct csm_shader_t), shader_map,
                nshaders * sizeof(struct csm_shader_t));
    free(shader_map);
  }
...
csm_shader_entry* load_all_entries(const char* shader_list_file_path, int* count)
{
  ...
  for (int i = 0;; i++)
  {
    ...
    if (strncmp(line, "//", 2) == 0)
    {
      sscanf(line, "// output: %s stage: %s name: %s", entry->output_path, entry->stage,
             entry->name);
    }
    else
    {
...
int          compile_shader(const struct csm_shader_entry* entry)
{
  ...
  luna_snprintf(g_Buffer, 1024, "%s %s %s -o %s -S %s", shader_compiler, shader_compiler_args,
                entry->path, strcmp(entry->output_path, "") != 0 ? entry->output_path : "",
                entry->stage);
...
void csm_compile_from_cache(csm_shader_entry* entries, int nentries,
                            csm_shader_cache_entry* cacheentries, int cachecount)
{
  for (int i = 0; i < nentries; i++)
  {
...
bool fontc_atlas_add_image(fontc_atlas_t* __restrict__ atlas, int w, int h,
                           const unsigned char* __restrict__ data, int* __restrict__ x,
                           int* __restrict__ y)
{
  const int padding = 4;
  ...
  for (int y = 0; y < h; y++)
  {
    luna_memcpy(atlas->data + (atlas->next_x + (atlas->next_y + y) * atlas->width), SIZE_MAX,
                data + (y * w), w);
  }
...
void fontc_read_font(const char* path, fontc_file* file)
{
  ...
  luna_decompress_data(compressed_glyphs, file->header.glyphs_compressed_sz, compressed_glyphs,
  ...
  cassert(luna_decompress_data(recv, file->header.img_compressed_sz, file->bitmap,
...
void fontc_bake_font(const char* font_path, const char* out)
{
  ...
  size_t       glyph_o_size;
  fontc_glyph* compressed_glyphs = luna_malloc(file.header.numglyphs * sizeof(fontc_glyph));
  luna_compress_data((const unsigned char*) file.glyphs,
                     file.header.numglyphs * sizeof(fontc_glyph),
                     (unsigned char*) compressed_glyphs, &glyph_o_size);
