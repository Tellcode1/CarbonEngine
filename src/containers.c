...
{
  // we don't remove all data here to reduce allocations but this can POSSIBLY hold tons of garbage
  // data
  vec->m_size = 0;
}
...
void cg_vector_insert(cg_vector_t* __restrict vec, int index, const void* __restrict elem)
{
  ...
  {
    cg_vector_resize(vec, cmmax(1, index * 2)); // linear allocator. i could probably implement more
                                                // but i don't have time rn
  }
  if (index >= vec->m_size)
  {
    vec->m_size = index + 1;
...
void cg_string_remove(cg_string_t* str, int index, int length)
{
  ...
  luna_memmove(str->data + index, str->length - index - length + 1, str->data + index + length,
...
cg_hashmap_t* cg_hashmap_init(int init_size, int keysize, int valuesize, cg_hashmap_hash_fn hash_fn,
                              cg_hashmap_key_equal_fn equal_fn)
{
