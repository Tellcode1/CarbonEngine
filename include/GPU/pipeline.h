...
#define CVK_REQUIRED_PTR(ptr)                                                                                                                        \
  if ((ptr) == NULL)                                                                                                                                 \
  LOG_AND_ABORT(#ptr " :  Required parameter \"" #ptr "\" specified as NULL.", __basename(__FILE__), __LINE__, __PRETTY_FUNCTION__)
#define CVK_NOT_EQUAL_TO(val, to)                                                                                                                    \
  if ((val) == (to))                                                                                                                                 \
  LOG_AND_ABORT(#val " == " #to ". Value \"" #val "\" must not be equal to " #to ".", __basename(__FILE__), __LINE__, __PRETTY_FUNCTION__)
...
static void __cvk_defaultResultCheckFunc(const VkResult result, const char *FILE, const char *FUNC, unsigned long LINE) {
  if (result != VK_SUCCESS)
    LOG_ERROR("[%s:%lu] %s: func returned %i", FILE, LINE, FUNC, result);
}
...
#define luna_GPU_InitPipelineCreateInfo()                                                                                                            \
  (luna_GPU_PipelineCreateInfo) {                                                                                                                    \
...
#define luna_GPU_InitRenderPassCreateInfo()                                                                                                          \
  (luna_GPU_RenderPassCreateInfo) {                                                                                                                  \
