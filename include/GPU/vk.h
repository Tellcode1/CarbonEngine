...
#define NOVA_VK_ALLOCATOR &vkallocator
#endif
...
static inline void*
vkalloc(void* pUserData, size_t size, size_t alignment, VkSystemAllocationScope allocationScope)
{
  NVAllocator alloc;
  NVAllocatorBindHeapAllocator(&alloc, &heap);
  return poolmalloc(&alloc, alignment, size);
}

static inline void*
vkrealloc(void* pUserData, void* pOriginal, size_t size, size_t alignment, VkSystemAllocationScope allocationScope)
{
  NVAllocator alloc;
  NVAllocatorBindHeapAllocator(&alloc, &heap);
...
static VkAllocationCallbacks vkallocator = { .pUserData = NULL, .pfnAllocation = vkalloc, .pfnReallocation = vkrealloc, .pfnFree = vkfree };
...
extern void NV_VK_CreateBuffer(
...
extern void     NV_VK_CreateTextureEmpty(
