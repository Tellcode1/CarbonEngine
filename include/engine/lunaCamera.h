...
// replace with a per camera variable!!!
...
static inline vec2 lunaCamera_GetMouseGlobalPosition(const lunaCamera *cam) {
  return v2add(v2mulv(lunaInput_GetMousePosition(), (vec2){CAMERA_ORTHO_W, CAMERA_ORTHO_H}), (vec2){cam->position.x, cam->position.y});
}
